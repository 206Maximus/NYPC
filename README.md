    ...
    def calculate_bid(self, dice_a: List[int], dice_b: List[int]) -> Bid:
        ...
        if self.round >= 9:
            ...
            if a_is_must_win or b_is_must_win:
                aggressive_late_bid = max(1, int(avg_opp_bid * 1.5))
                group_to_bid = 'A' if my_potential_a >= my_potential_b else 'B'
                return Bid(group_to_bid, aggressive_late_bid)
            else:
                group_to_bid = 'A' if my_potential_a >= my_potential_b else 'B'
        ...
        # 초중반 라운드 전략 (2-8 라운드)
            ...
            critical_bid = max(1, int(avg_opp_bid * 1.4))
            ...
            if my_a_is_prio or my_b_is_prio:
                # [수정됨] 상대가 공격적일 때 추가 베팅하는 로직
                # 상대 평균 베팅액이 800을 넘고, 입찰에서 손해를 보지 않았다면 공격적인 상대로 간주
                is_opp_aggressive = avg_opp_bid > 800
                is_opp_winning_bids = self.opp_state.bid_score <= 0
                
                if is_opp_aggressive and is_opp_winning_bids:
                    critical_bid += 3000  # 3000점 추가 베팅
        ...
        for rule in priority_order:
            if rule in available_rules:
                ...
                for combo in dice_combos:
                    ...
                    if six_rule_is_available and rule in rules_to_save_six and 6 in dice_list:
                        continue
